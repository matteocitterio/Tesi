...
quant='AR'
divisors=[115,800,180,140,30,30,50,15,145,15]
ar_ratio=[]
gdl=[]
ar_means=[]
ar_sigmas=[]
...
for eta in range(0,len(img_ns_normal_string)): 
    ...
    dj=pd.read_csv(r'D:\matteo_citterio\Python Scripts\Area_ratio_per_image\Area_ratio_'+img_ns_normal_string[eta]+'.csv')
    ...
    if (img_ns_string[eta]=='9_INT_Normal'):
        ...
        params2=stats.lognorm.fit(data, loc=1)
    ...
    ax.plot(x,hist_dist.pdf(x),'-o', label='data PDF'+img_ns_normal_string[eta],lw=3)
    ...
    ax.set_title('Area distributions')
    ax.set_xlim(-0.1,6)
