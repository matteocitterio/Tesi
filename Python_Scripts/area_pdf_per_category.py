...
img_ns_tumor_string=['9_INT_Tumor','13_INT_Tumor','5_INT_Tumor','7_INT_Tumor','8_INT_Tumor']
real_img_n_tumor=[ 18, 9, 13, 6, 6]
...
for eta in range(0,len(img_ns_tumor_string)): 
    ...
    df=pd.read_csv(r'D:\matteo_citterio\Python Scripts\Area_ratio_per_image\Area_ratio_'+img_ns_tumor_string[eta]+'.csv')
...
for eta in range(0,len(img_ns_normal_string)): 
    ...
    df=pd.read_csv(r'D:\matteo_citterio\Python Scripts\Area_ratio_per_image\Area_ratio_'+img_ns_normal_string[eta]+'.csv')
...
tumor_restricted=[]
for i in range(0,len(tumor)):
    if(tumor[i]<200 and tumor[i]>2):
        tumor_restricted.append(tumor[i])

tumor=tumor_restricted
...
params_tum=stats.gamma.fit(tumor, loc=0)

if (img_ns_string[eta]=='9_INT_Normal'):

    params2=stats.lognorm.fit(tumor)

else:

    params2=stats.lognorm.fit(tumor, loc=0)

# print(params)
print(params_tum)
gdl.append(params2[0])

pdf_fitted =stats.gamma.pdf(x, *params_tum)
...
bx.plot(x, pdf_fitted, color='r',label='lognorm fit',lw=3)
...
ax.set_title('Tumor area distribution')

bx.set_xlabel('area')
bx.set_ylabel('Normalized counts')
bx.set_title('Tumor area distribution LOG')
...
for i in range(0,len(normal)):
    if(normal[i]<100):
        normal_restricted.append(normal[i])

normal=normal_restricted
...
params_norm=stats.gamma.fit(normal, loc=0)

if (img_ns_string[eta]=='9_INT_Normal'):

    params2=stats.lognorm.fit(normal)

else:

    params2=stats.lognorm.fit(normal, floc=0)

# print(params)
print(params_norm)
gdl.append(params2[0])

pdf_fitted =stats.gamma.pdf(x, *params_norm)
pdf_fitted2 =stats.lognorm.pdf(x, *params2)
ax.plot(x,hist_dist.pdf(x),'-o', label='data PDF',lw=3)
ax.plot(x, pdf_fitted, color='r',label='lognorm fit',lw=3)
ax.plot(x, pdf_fitted2, color='k',label='lognorm fit',ls='--',lw=3)

bx.plot(x,hist_dist.pdf(x),'-o', label='data PDF',lw=3)
bx.plot(x, pdf_fitted, color='r',label='lognorm fit',lw=3)
bx.plot(x, pdf_fitted2, color='k',label='lognorm fit',ls='--',lw=3)
bx.set_yscale('log')
    # bx.set_xscale('log')
bx.legend()
...
ax.set_xlabel('area')
ax.set_ylabel('Normalized counts')
ax.set_title('Normal area distribution')

bx.set_xlabel('area')
bx.set_ylabel('Normalized counts')
bx.set_title('Normal area distribution LOG')
...
ax.set_xlabel('(area-1)/(<area>-1)')
ax.set_ylabel('PDF')
params=stats.lognorm.fit(normal_normalized,loc=0)
params2=stats.lognorm.fit(turmor_normalized,loc=0)
...
print('with lognorm cdf tumor',stats.ks_1samp(tumor,cdf=stats.lognorm.cdf,args=params_tum)[1])
print('with lognorm cdf normal',stats.ks_1samp(normal,cdf=stats.gamma.cdf,args=params_norm)[1])
